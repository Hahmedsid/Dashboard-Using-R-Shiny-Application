...
ui <- dashboardPage(title = 'Global Terrorism Statistics Dashboard', header, sidebar, body, skin='red')    
...
server <- function(input, output) { 
  ...
  output$killbyregion <- renderPlot({
    ...
    ggplot(data = data_filter, 
      ...
      geom_bar(position = "dodge", stat = "identity", fill="red") + ylab("Count of kills") + 
  ...
  output$Killbyattacktype <- renderPlot({
    data_region <- datasetregion()
      ...
      geom_bar(position = "dodge", stat = "identity", fill="black") + ylab("Count of Kills") + 
  ...
  output$targetkilled <- renderPlot({
    data_target <- datasettarget()
      ...
      geom_bar(position = "dodge", stat = "identity", fill="steelblue") + ylab("Count of Kills") + 
      xlab("Target Type") + theme(legend.position="bottom" 
                              ,plot.title = element_text(size=15, face="bold")) + 
